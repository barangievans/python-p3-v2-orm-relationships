...
class TestDepartment:
    ...
    def drop_tables(self):
        ...
        Department.create_table()  # Ensure the table is created after dropping
        Department.all = {}

    def test_creates_table(self):
        '''contains method "create_table()" that creates table "departments" if it does not exist.'''
        Department.create_table()
        CURSOR.execute("SELECT * FROM departments")  # Make sure to actually fetch results
        assert CURSOR.fetchone() is None  # There should be no rows initially
    ...
    def test_deletes_row(self):
        ...
        assert department2.id is None  # Ensure id is set to None after deletion
        assert Department.all.get(id2) is None  # Ensure dictionary entry was deleted
